			...
			case Mgmt::ECommandCompleteEvent:
			{
					...
					switch(event.commandCode)
				{
					case Mgmt::EReadAdvertisingFeaturesCommand:
					{
						...
						// flags[4], max_adv_data_len[1], max_scan_rsp_len[1], instances[1].
						if (event.status == 0 && dataLen >= 7 && data[6] != 0)
						{
							supportedAdvertisingInstances = data[6];
...
{
	std::lock_guard<std::mutex> sendLock(commandSendMutex);
	// Auto-connect
